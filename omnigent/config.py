...
def github_account_preference(repo_nwo: str, path: Path | None = None) -> str | None:
    """Return the preferred ``gh`` login for a base repo, or ``None``.

    :param repo_nwo: Base repository as ``owner/name`` (matched case-insensitively).
    :param path: Config path override (defaults to :func:`global_config_path`).
    """
    accounts = _github_accounts(load_global_config(path))
    value = accounts.get(repo_nwo.lower())
...
def set_github_account_preference(
    ...
    """Persist (or clear, when *login* is falsy) the preferred login for a repo.

    Stored under ``github.accounts`` keyed by the lower-cased ``owner/name`` so a
    personal account choice follows the repo across worktrees and clones.

    :param repo_nwo: Base repository as ``owner/name``.
